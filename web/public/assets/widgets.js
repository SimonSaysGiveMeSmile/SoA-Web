...
import { t as tr } from '/assets/i18n.js?v=10';
import { getSettings } from '/assets/settings.js?v=10';
...
async function _loadGlobeAssets() {
    ...
    _globeAssetsPromise = (async () => {
        ...
        if (!window.THREE) {
            await _loadScript('/assets/vendor/three.min.js?v=10');
        }
        if (!window.ENCOM || !window.ENCOM.Globe) {
            await _loadScript('/assets/vendor/encom-globe.js?v=10');
        }
        const gridResp = await fetch('/assets/vendor/grid.json?v=10', { credentials: 'same-origin' });

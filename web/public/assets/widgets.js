...
import { t as tr } from '/assets/i18n.js?v=11';
import { getSettings } from '/assets/settings.js?v=11';
...
async function _loadGlobeAssets() {
    ...
    _globeAssetsPromise = (async () => {
        ...
        if (!window.THREE) {
            await _loadScript('/assets/vendor/three.min.js?v=11');
        }
        if (!window.ENCOM || !window.ENCOM.Globe) {
            await _loadScript('/assets/vendor/encom-globe.js?v=11');
        }
        const gridResp = await fetch('/assets/vendor/grid.json?v=11', { credentials: 'same-origin' });

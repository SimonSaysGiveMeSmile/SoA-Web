...
// ── REAL usage-limit utilization ──────────────────────────────────────────
// The `week`/`block` fields above are a rolling-window estimate derived from
// local transcripts; they diverge from Anthropic's fixed limit blocks (the
// numbers `/usage` shows). `scripts/soa-usage-limits` fetches the ground truth
// from the OAuth usage endpoint into $STATE_DIR/usage-limits.json; we read that
// file and, when it goes stale, kick a detached refresh (non-blocking). If the
// file is missing/old the client falls back to the rolling estimate.
const LIMITS_STATE_DIR = process.env.SOA_WEB_STATE_DIR || path.join(os.homedir(), '.soa-web-local');
const LIMITS_FILE = path.join(LIMITS_STATE_DIR, 'usage-limits.json');
const LIMITS_SCRIPT = path.join(__dirname, '..', '..', 'scripts', 'soa-usage-limits');
const LIMITS_FRESH_MS = 60 * 1000;      // consider the cache fresh for 60s
const LIMITS_SPAWN_GAP_MS = 45 * 1000;  // don't launch the refresher more than ~1/45s
let _limitsSpawnedAt = 0;

function readLimits() {
    let data = null, ageMs = Infinity;
    try {
        const raw = fs.readFileSync(LIMITS_FILE, 'utf8');
        data = JSON.parse(raw);
        ageMs = Date.now() - (data.fetchedAt || 0);
    } catch (_) { /* missing/corrupt — treat as stale */ }
    // Kick a background refresh when stale (rate-limited so rapid polls don't fork).
    if (ageMs > LIMITS_FRESH_MS && (Date.now() - _limitsSpawnedAt) > LIMITS_SPAWN_GAP_MS) {
        _limitsSpawnedAt = Date.now();
        try {
            const child = require('child_process').spawn(LIMITS_SCRIPT, [], {
                detached: true, stdio: 'ignore',
                env: Object.assign({}, process.env, { SOA_WEB_STATE_DIR: LIMITS_STATE_DIR }),
            });
            child.on('error', () => {});
            child.unref();
        } catch (_) { /* refresher unavailable — serve whatever's cached */ }
    }
    if (!data) return null;
    return { fiveHour: data.fiveHour || null, sevenDay: data.sevenDay || null,
             sevenDayOpus: data.sevenDayOpus || null, fetchedAt: data.fetchedAt || null, ageMs };
}

// Public snapshot with a short throttle so rapid polls are cheap.
function snapshot() {
    const now = Date.now();
    if (_lastComputed && (now - _lastAt) < MIN_RECOMPUTE_MS) {
        // Limits come from a separate cache file that refreshes on its own
        // cadence — refresh that view even on a memoized snapshot so the widget
        // reset-countdown stays live without recomputing the whole transcript scan.
        return Object.assign({}, _lastComputed, { limits: readLimits() });
    }
    _lastComputed = compute();
    _lastComputed.limits = readLimits();
    _lastAt = now;
